...
```{r photo, fig.margin = TRUE, echo = FALSE, fig.width = 3, fig.cap = "Photo by Jovana Askrabic on Unsplash", fig.alt = "A dog looking up at the camera.", eval = TRUE}
...
```{r clone-repo-link, fig.margin = TRUE, echo = FALSE, eval = TRUE, fig.alt = "GitHub repository page with the green Code button open, showing the HTTPS clone URL and a clipboard icon to copy it."}
...
Go to [JuptyerHub](https://santiago.cloudbank.2i2c.cloud/hub/login) and then **open an RStudio Notebook**.

```{r course-workspace, fig.margin = TRUE, echo = FALSE, eval = TRUE, fig.alt = "JupyterHub launcher screen showing the RStudio Notebook option."}
knitr::include_graphics("img/jupyterhub.png")
```
...
```{r new-project-from-git, echo = FALSE, eval = TRUE, fig.align = "left", fig.alt = "RStudio's New Project from Git Repository dialog with the repository URL field highlighted."}
...
```{r rstudio-anatomy, fig.fullwidth=TRUE, echo = FALSE, eval = TRUE, fig.alt = "Diagram labeling RStudio's four panes: Console (bottom left), Files/other panes (bottom right), Editor (top left), and Environment/Git (top right)."}
...
```{r yaml-raw-to-rendered, echo = FALSE, eval = TRUE, fig.align = "center", fig.alt = "Side-by-side comparison of the raw YAML header text and how the author name appears in the rendered document title block."}
...
```{r update-author-name-commit, echo = FALSE, eval = TRUE, fig.align = "left", fig.alt = "RStudio Git commit window showing the changed files checked and the commit message 'Update author name' entered."}
...
```{r ready-to-push, fig.margin = TRUE, echo = FALSE, eval = TRUE, fig.alt = "RStudio Git pane with the Push button highlighted, ready to send committed changes to GitHub."}
...
```{r load-packages-chunk, echo = FALSE, eval = TRUE, fig.align = "left", fig.alt = "RStudio code chunk showing library(tidyverse) and library(openintro) with the Run Current Chunk icon highlighted."}
...
```{r data.viewer, echo = FALSE, eval = TRUE, fig.align = "left", fig.alt = "RStudio data viewer window showing the seattlepets dataset as a spreadsheet with columns for animal name and species."}
...
🧶 Knit → ✅ Commit with message `"Completed Exercise 2"` → ⬆️ Push
...
```{r eval = TRUE, echo = FALSE, fig.fullwidth = TRUE, warning=FALSE, fig.alt = "Scatterplot comparing the proportion of dogs versus cats sharing each of the 20 most common pet names, with a diagonal reference line where a name would be equally popular for both species; most points fall off the line, indicating names tend to skew toward one species."}
# data prep
...
**✓ Final Checkpoint:** Visit your GitHub repo one more time to confirm all your work is there. Grading is based on the PDF you upload to Canvas, so double-check it before submitting — but your GitHub repo should be up to date as well.
