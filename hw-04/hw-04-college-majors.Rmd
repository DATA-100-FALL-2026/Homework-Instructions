...
```{r photo, fig.margin = TRUE, echo = FALSE, fig.width = 3, fig.cap = "Photo by Marleena Garris on Unsplash", fig.alt = "Photo of a student in a graduation cap and gown.", eval = TRUE}
...
```{r lowest-unemp-select, eval = TRUE}
college_recent_grads %>%
  arrange(unemployment_rate) %>%
...
```{r income-histogram, fig.height = 2, eval = TRUE, fig.alt = "Histogram of median income across all majors in the dataset, showing a right-skewed distribution with most majors clustered at lower incomes and a long tail toward a few higher-earning majors."}
...
**Reminder:** once you fill in a chunk and change it to `eval = TRUE`, also fill in `fig.alt = "___"` in the chunk header with a short description of what your histogram shows.

```{r binwidth-1000, eval = FALSE, fig.alt = "___"}
# Try binwidth = 1000
```
```{r binwidth-5000, eval = FALSE, fig.alt = "___"}
# Try binwidth = 5000
```

We can also calculate summary statistics for this distribution using the `summarise` function:

```{r income-summary, eval = TRUE}
college_recent_grads %>%
...
```{r income-by-category, eval = FALSE, fig.alt = "___"}
...
```{r highest-income-category, eval = FALSE}
college_recent_grads %>%
...
```{r add-major-type, eval = TRUE}
college_recent_grads <- college_recent_grads %>%
  mutate(major_type = ifelse(major_category %in% stem_categories, "stem", "not stem"))
```

Let's unpack this: with `mutate` we create a new variable called `major_type`, which is defined as `"stem"` if the `major_category` is in the vector called `stem_categories` we created earlier, and as `"not stem"` otherwise. Notice the `college_recent_grads <-` at the start — that saves the new column back into `college_recent_grads`. If it were missing, `major_type` would print once and then disappear, and Exercise 8 below would fail with a "column not found" error.
...
```{r women-income-stem, eval = FALSE, fig.alt = "___"}
...
```{r custom-analysis}
# Your code here
# If your analysis includes a plot, add fig.alt = "..." to this chunk header
# describing what the plot shows.
```
...
- **Error: "object 'major_type' not found"** → Make sure you ran the `mutate()` code that creates the `major_type` variable, and that it reassigns back with `college_recent_grads <- college_recent_grads %>% mutate(...)`
...
**✓ Final check:** Visit your GitHub repo to confirm all your commits are there — this is your checkpoint/backup. The PDF you upload to Canvas is the basis for grading, so double-check it's complete and readable before submitting.
