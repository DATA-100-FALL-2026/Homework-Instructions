...
Questions like this one are sensitive, and how a survey measures them affects how trustworthy the resulting statistic is. As you work through Questions 1-2, keep in mind that the percentage you calculate is only as good as the survey design and response behavior behind it (e.g. who feels comfortable answering, who skips the question, how the question is worded).
...
4.  Visualize the distribution of this new variable. Find the mean and the median number of minutes respondents spend on email weekly. Is the mean or the median a better measure of the typical amount of time Americans spend on email weekly? Why?
...
**Hint:** A complete interpretation of a slope states (a) the units, (b) the direction of the association, and (c) for a categorical predictor like `wrkstat` or `snap_insta`, which baseline level the comparison is relative to. For example: "Holding `wrkstat` and `snap_insta` fixed, each additional year of education is associated with an estimated change of ___ minutes per week spent on email." For `wrkstat`, state the comparison group explicitly (e.g., "...compared to respondents who are working full time").

9.  Create a predicted values vs. residuals plot for this model. Are there any issues with the model? If yes, describe them.

**Hint:** A residual plot with no major issues shows points scattered randomly around the horizontal line at 0, with roughly constant spread across the range of fitted values. Watch for: a curved pattern (suggests the relationship isn't linear), a funnel shape where spread grows or shrinks with fitted values (suggests non-constant variance), or a small number of points far from the rest (potential outliers).
...
11. In a new variable, recode `polviews` such that Extremely liberal, Liberal, and Slightly liberal, are mapped to `"Liberal"`, and Slghtly conservative, Conservative, and Extrmly conservative are mapped to `"Conservative"`. The remaining levels can be left as is. Make sure that the levels are in a reasonable order. Don't overwrite the existing `polviews`, instead pick a different, informative name for your new variable.
...
**Putting it all together:** People sometimes claim that political conservatives are less supportive of government-funded science research than liberals or moderates. Based on the relationship you found between your recoded `polviews` and `advfront` variables, does the 2016 GSS data support, complicate, or contradict this claim? Point to specific patterns in your visualization to justify your answer.
