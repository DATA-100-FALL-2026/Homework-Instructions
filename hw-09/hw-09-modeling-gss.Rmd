...
```{r recode-advfront, eval = FALSE}
gss16 <- gss16 %>%
  mutate(advfront = case_when(
    advfront %in% c("Strongly agree", "Agree") ~ ___,
    is.na(advfront) ~ ___,
    TRUE ~ ___  # All other cases
  ))

# Reorder levels
gss16 <- gss16 %>%
  mutate(advfront = factor(advfront, levels = c(___, ___)))
...
7.  Build a workflow that uses the recipe you defined (`gss16_rec_1`) and the model you specified (`gss16_spec`). Name this workflow `gss16_wflow_1`. Sample code is provided below.
...
> **Putting it together:** If Model 2 (without `wrkstat`) performs about as well as Model 1 on the testing data, that's a meaningful result, not a disappointing one. It suggests `wrkstat` wasn't adding real predictive information — it was just adding complexity. A simpler model that generalizes just as well is generally preferable to a more complex one that doesn't, since the complex model is more prone to overfitting patterns specific to the training data. Keep this idea in mind: "simpler but equally good" is a legitimate modeling win, not a consolation prize.
...
> **What a complete answer covers:** A full description should walk through all three steps: (1) draw a large number of resamples from the original sample, each the same size as the original sample and drawn *with replacement*; (2) calculate the proportion of "Yes" responses within each resample; (3) use the spread of proportions across all the resamples to estimate how much our proportion would vary if we could have drawn many different samples from the population. Simply saying "resample with replacement" isn't enough — explain *why* that lets us estimate uncertainty without needing to survey the population again.
...
> **Interpretation template:** Follow the same phrasing pattern from earlier confidence interval work: "We are 95% confident that the true proportion of American adults who have experienced workplace harassment by a superior or co-worker is between ___ and ___." Avoid saying there's a "95% probability" the true proportion falls in this range — that's a common but incorrect phrasing. The 95% describes the *method's* long-run success rate, not a probability about this one interval.
