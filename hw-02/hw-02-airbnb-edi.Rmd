...
```{r photo, fig.margin = TRUE, echo = FALSE, fig.width = 3, fig.cap = "Photo by Madeleine Kohler on Unsplash", fig.alt = "A row of colorful traditional buildings, evoking the residential neighborhoods of Edinburgh discussed in this assignment.", eval = TRUE}
...
**Your answer:** [Write a sentence here using inline R code, e.g., the number of observations is \`r nrow(edibnb)\`, so the number updates automatically if the data changes.]
...
```{r price-hist-experiments, fig.alt = "Faceted histograms of Airbnb price by neighborhood, tested at binwidths of 25, 50, and 100."}
# Try binwidth = 25
...
**Your answer:** [Which binwidth did you choose and why?]
...
**Your answer:** [Which layout did you choose and why?]
...
```{r final-price-histogram, eval = FALSE, fig.alt = "Faceted histogram of Airbnb price by neighborhood, using the binwidth and layout chosen above."}
...
```{r ridge-plots, message = FALSE, eval = FALSE, fig.alt = "Ridge plot comparing the distribution of Airbnb prices across the top 5 most expensive neighborhoods."}
library(ggridges)
...
**Your answer:** [Type your answer here. Use as much space as you need.]
...
```{r review-scores-viz, eval = FALSE, fig.alt = "Visualization comparing the distribution of Airbnb review scores across neighborhoods."}
# Your code here
```
...
- **Can't filter for top 5 neighborhoods** → Make sure you're using `top_5$neighbourhood` to reference the neighborhood names, and that `top_5` was created with `top_5 <- edibnb %>% ...` (not just piped without an assignment)
...
**✓ Final check:** The PDF you upload to Canvas is what gets graded, so double-check it before submitting — make sure every code chunk ran without errors and all your answers and plots show up in the output. Also visit your GitHub repo to confirm your commits are there; both the PDF and the GitHub push are required, but grading is based on the PDF.
